...
const Quote = () => {
        ...
        const [myQuote, setMyQuote] = useState(values[0].text)


        const updateQuote = () => {
            setMyQuote(values[getRandomInt(0,50)].text)
        }


    return(
        <div className="container">
            <GiGoose id="goose" onClick={() => updateQuote()}/>
            <h3 className="quote">{myQuote}</h3>
            <button className="button" onClick={() => updateQuote()}>Click Me</button>
        </div>
    )
}
